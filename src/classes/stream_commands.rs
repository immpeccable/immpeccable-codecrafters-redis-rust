        ...
        match latest_id {
            Some(latest_id) => {
                ...
                if miliseconds_time <= last_miliseconds_time {
                    current_sequence_number = last_sequence_number.parse::<u32>().unwrap() + 1u32
                }
...
pub async fn handle_xadd(
    ...
    if commands.len() >= 3 {
        ...
        drop(guard);
        // Propagation to replicas should be handled in the dispatcher for now
    }
}
                ...
                for entry in result {
                    ...
                    for (key, value) in entry {
                        if key == "id" {
                            ...
                            keys_values_except_id.push(RespDataType::BulkString(key.to_string()));
                            keys_values_except_id.push(RespDataType::BulkString(value.to_string()));
                        }
                    }
                    result_resp_string.push_str(&format!("{}", RespDataType::BulkString(id)));
                    result_resp_string.push_str(&format!("{}", RespDataType::Array(keys_values_except_id)));
                }
                stream
                    .lock()
                    .await
...
pub async fn handle_xread(
    ...
    if let Some(second_command) = commands.get(1) {
        if second_command.to_uppercase() == "BLOCK" {
            has_block = true;
            if let Some(block_time_string) = commands.get(2) {
